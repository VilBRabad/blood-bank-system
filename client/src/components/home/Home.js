...
function Home(props) {
   ...
   const [storeType, setStoreType] = useState("lab");
   ...
   useEffect(()=>{
      let Type = storeType;
      console.log(Type)
      setLoginData({type:Type})
   }, [storeType])
   
   const postLoginData = async(e)=>{
      e.preventDefault();
      const {username, password, type} = loginData;
   ...
   const postLabResisterData = async(e)=>{
      ...
      if(id.length===6 && username.length < 10){
         //console.log(username, name, id, mobile, address, pincode);
         ...
         if(data.code === 200){
            ...
            window.alert("ID/username Aready EXISTS.!!");
         }
         else if(data.code === 500){
            window.alert("Internal Server Error!!");
         }
         else if(data.code === 405){
            window.alert("Username must be less than size of 10")
         }
         else{
            window.alert("Server ERROR!");
         }
         ...
         window.alert("INVALID Username/ID");
      }
   ...
   const searchClick=(e)=>{
      ...
      if(pinCode.length === 6){
         navigate(`/Search-Blood/${pinCode}`)
      }
      else{
         window.alert("INVALID pincode Number....");
      }
   }
            ...
            <div className="flex rel">
               ...
               <form>
                  <div className="formDiv flex">
                     <input type="number" name="pinCode" value={pinCode} onChange={handleSearchChange} placeholder="Search Blood (pincode e.g. 401000)" required/>
